...
export default function ChatDetailScreen(props: {
  ...
  useEffect(() => {
    ...
    if (isRecording && !isPausedRecording) {
      ...
      // Waveform (visual bars every 120ms)
      waveInterval = setInterval(() => {
        setRecordingBars(prev => {
          const newVal = Math.random() > 0.3 ? (Math.random() * 18 + 6) : (Math.random() * 6 + 2);
          return [...prev, newVal].slice(-30);
        });
      }, 120);
    }
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: 'rgba(255,255,255,0.7)', fontFamily: 'SF Pro Text, monospace' }}>
                {isPlaying ? `0:${Math.floor((voiceProgress / 100) * parseInt(msg.content.split(':')[1])).toString().padStart(2,'0')}` : msg.content}
              </span>
              <button 
                onClick={(e) => {
                  e.stopPropagation();
      ...
      <div style={{
        ...
        <div 
          ...
          style={{
            ...
            borderRadius: isRecording ? 18 : 27,
            background: isRecording ? 'rgba(30,30,35,0.8)' : (showSearch ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.12)'),
        ...
        >
          {showSearch ? (
            ...
            /* DYNAMIC VOICE RECORDING UI */
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, height: '100%', padding: '0 8px' }}>
              
              <button 
                onClick={handleCancelRecording}
                style={{ 
                  width: 34, height: 34, borderRadius: '50%', background: 'rgba(255,255,255,0.12)', 
                  border: 'none', display: 'flex', justifyContent: 'center', alignItems: 'center', 
              ...
              <button 
                ...
                style={{ 
                  width: 34, height: 34, borderRadius: '50%', background: isPausedRecording ? '#FFB800' : 'rgba(255,255,255,0.12)', 
                  border: 'none', display: 'flex', justifyContent: 'center', alignItems: 'center', 
                  cursor: 'pointer', flexShrink: 0, transition: 'all 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275)'
                }}
              >
                {isPausedRecording ? (
                  /* Play/Continue Icon */
              ...
              <div style={{ flex: 1, height: 36, display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,0.08)', borderRadius: 12, padding: '0 10px', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, marginRight: 8 }}>
                  <div style={{ width: 6, height: 6, borderRadius: '50%', backgroundColor: isPausedRecording ? 'rgba(255,255,255,0.3)' : '#FF3B30', animation: !isPausedRecording ? 'pulse 0.8s infinite' : 'none' }} />
                </div>
                
                <div style={{
                  flex: 1, height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 2,
                  overflow: 'hidden',
              ...
              <div style={{ flexShrink: 0, minWidth: 44, display: 'flex', justifyContent: 'center' }}>
              ...
              <button 
                ...
                onPointerUp={e => e.currentTarget.style.transform = 'scale(1)'}
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7" /></svg>
              </button>
            </div>
          ) : (
