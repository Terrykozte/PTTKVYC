/**
 * MapsArchiveView.tsx  (v5 — Individual Pin Markers)
 *
 * – CartoDB Voyager tile map
 ...
 * – SAME-LOCATION STACKING: pins at exact same coordinates show count badge
 *   and horizontal photo scroll in preview card
 * – INDIVIDUAL PIN DISPLAY: all pins are shown separately, no proximity clustering.
 * – Zoom into locations manually to separate overlapping pins.
...
// Strictly SQUARE photo pin: image + inline address text + precise triangle pointer
// Total height: 4 (pad) + 64 (img) + 4 (gap) + 26 (text 2 lines) + 4 (pad) = 102 card
//             + 8 (triangle) - 1 (overlap) = 109
// iconAnchor Y = 109 → exact geo coord sits at the triangle tip
function makePhotoIcon(
  ...
  const sh = hl ? 'rgba(255,214,10,0.55)' : 'rgba(0,0,0,0.38)';
  const borderColor = hl ? '#FFD60A' : 'rgba(0,0,0,0.08)';
  const borderWidth = hl ? 2 : 1;
  const pointerColor = hl ? '#FFD60A' : '#ffffff';
  ...
  return L.divIcon({
    ...
    html: `<div style="display:flex;flex-direction:column;align-items:center;animation:pinPop 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) ${delay}s both;">
      <div style="
        ...
        box-shadow: 0 8px 24px ${sh}, 0 0 0 1px rgba(0,0,0,0.04);
        position: relative;
        width: 72px;
        cursor: pointer;
        box-sizing: border-box;
      ">
        <div style="width: 64px; height: 64px; overflow: hidden; background: #e5e5e5;">
...
function AutoBounds({
  ...
  useEffect(() => {
    if (focus) {
      map.flyTo([focus.lat, focus.lng], 14, { duration: 0.7 });
...
export default function MapsArchiveView({
  historyItems,
  viewerIdentity,
  onClose,
  initialFocusLocation,
}: MapsArchiveViewProps) {
  ...
  // 1. Filter photos by toggle
  const displayPhotos = historyItems.filter((item) => {
    if (!item.location || !item.image) return false;
    if (mapFilter === 'you')
      return item.sender === viewerIdentity || item.sender === 'You';
    return true;
  });

  // 2. Each photo is its own pin (no grouping by location name)
  const pins: LocationPin[] = useMemo(() => {
    return displayPhotos.map((item) => {
  ...
  const onPtrMove = (e: React.PointerEvent) => {
    ...
    const now = Date.now();
    dragRef.current.vel =
      (e.pageY - dragRef.current.lastY) / Math.max(1, now - dragRef.current.lastT);
  ...
  return (
    <div
      style={{
        ...
        inset: 0,
        zIndex: 8000,
        background: `rgba(0,0,0,${Math.max(0, 0.85 - sheetOffset * 0.002)})`,
        ...
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
          ...
          {!selectedPin && (
            ...
            >
                ...
                {(['you', 'all'] as const).map((type) => (
                  <button
                    ...
                    style={{
                      ...
                      letterSpacing: 0.5,
                      background: mapFilter === type ? 'white' : 'transparent',
                      color:
                        mapFilter === type ? '#000' : 'rgba(255,255,255,0.6)',
              ...
              <button
                onClick={() =>
                  setFlyTarget({ lat: MY_POS[0], lng: MY_POS[1] })
                }
                style={{
                  width: 44,
                  height: 44,
          ...
          {/* Header — solid frosted-glass pill */}
          <div
            style={{
              position: 'absolute',
              top: `calc(18px + env(safe-area-inset-top, 40px))`,
              left: 12,
              right: 12,
              zIndex: 1200,
            }}
          ...
          {selectedPin && (
            <div
              style={{
                ...
                overflow: 'hidden',
                animation: 'cardUp .32s cubic-bezier(.16,1,.3,1)',
              }}
              onClick={(e) => e.stopPropagation()}
            >
              <button
              ...
              >
                ...
                <div
                  style={{
                    ...
                    inset: 0,
                    background:
                      'linear-gradient(to bottom, rgba(0,0,0,.15) 0%, transparent 35%, rgba(0,0,0,.72) 100%)',
                  }}
                />
                <div
                  style={{
                      ...
                      style={{
                        ...
                        border:
                          idx === selectedPhotoIdx
                            ? '2.5px solid #FFD60A'
                            : '2.5px solid rgba(255,255,255,.08)',
                        transition: 'border-color .2s ease',
                        boxShadow:
                          idx === selectedPhotoIdx
                            ? '0 0 10px rgba(255,214,10,.35)'
                            : 'none',
                      }}
                    >
                      <img
